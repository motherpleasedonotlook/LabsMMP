...
namespace Labs
{
    public partial class Double_simplex_form : Form
    {
        ...
        double[,] Matrix;
        string to_find;
        public Double_simplex_form(double[,] matrix, string to_find)
        {
            Matrix = matrix;
            InitializeComponent();
        ...
        private double[,] CalculationStep(DataGridView Table)
        {
            ...
            if (d_solutionRowIfNotOptimal(matrix) != -1)
            {
                ...
                solution_col = d_solutionColumnIfNotOptimal(matrix, solution_row);//получаем разрешающий столбец
            ...
            else
            {
                ...
                solution_col = isOptimal(matrix);//разрешающий столбец
        ...
        private int d_solutionRowIfNotOptimal(double[,] matrix)
        {
        ...
        private int d_solutionColumnIfNotOptimal(double[,] matrix, int row)
        {
            int height = matrix.GetLength(0);//по вертикали
            int wight = matrix.GetLength(1);//по горизонтали
        ...
        private string Interpretation(DataGridView Table)
        {
            ...
            if (to_find == "min")
                res += $" {(-1)*Convert.ToDouble(Table.Rows[table_rows_count - 1].Cells[table_columns_count - 1].Value)}.\n";
